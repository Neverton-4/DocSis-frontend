...
const NovoProtocoloFormWrapper = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (typeof fieldValue === 'string') {
      ...
      if (name === 'contato' && fieldValue.length <= 11) {
        ...
        // Formato: (11) 9999-9999 para números com 10 dígitos
  ...
  const handleServidorSelect = async (servidor: any) => {
    try {
      ...
      // Obter primeiro endereço se existir
      const primeiroEndereco = servidorCompleto.enderecos?.[0];
      ...
      setFormData(prev => {
        const newFormData = {
          ...
          // Mapear endereço do array enderecos
          ...
          // Mapear cargo e lotacao dos objetos aninhados
          cargo: servidorCompleto.cargo?.nome || servidorCompleto.cargo_2?.nome || '',
          lotacao: servidorCompleto.lotacao?.abrev || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let servidorId: string | number = formData.id;

      if (!servidorId) {
        const servidorExistente = await buscarServidorPorCPF(formData.cpf);
        if (servidorExistente) {
          servidorId = servidorExistente.id.toString();
        } else {
          const novoServidor = await salvarServidor(formData as any);
          servidorId = novoServidor.id.toString();
        }
      }

      const processoPayload = {
        servidor_id: parseInt(servidorId.toString()),
        usuario_id: user?.id,
        tipo_processo: formData.tipoProcesso_escolhido?.toLowerCase() || 'outro',
        nome: formData.tipoProcesso,
        detalhes: formData.tipoProcessoOutro || '',
        status: 'pendente'
      };

      console.log('Payload sendo enviado para a API:', processoPayload);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* Card de Processo - sem linha acima */}
      <Card className="p-4 bg-white">
        <h2 className="text-lg font-semibold mb-3">Processo</h2>
        <ProcessoForm
          formData={formData}
